...
import { ArrowLeft, CheckCircle, XCircle, FileText, Code, ListChecks } from '@phosphor-icons/react';
import { Button } from '@/components/ui/button';
import { useToast } from '@/components/ui/use-toast';
import { testsApi } from '@/lib/api';
import type { AttemptResultDetail } from '@/lib/api';

export default function MyTestResult() {
  ...
  if (!detail) return (
    <div className="page-container">
      <div className="neo-card p-12 text-center text-muted-foreground">Result not found or not yet published.</div>
    </div>
  );
  ...
  return (
    <div className="page-container max-w-4xl mx-auto">
      <div className="flex items-center gap-4 mb-8">
        <Button variant="ghost" size="icon" onClick={() => navigate('/my-results')} className="h-8 w-8">
          <ArrowLeft className="h-4 w-4" />
        </Button>
        <div>
          <h1 className="text-2xl font-medium tracking-tight text-foreground" style={{ fontFamily: 'var(--font-serif)' }}>Your Result</h1>
                  ...
                  return (
                    <div key={opt.id} className={`flex items-center justify-between p-3 rounded-md border ${styleClass}`}>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {isSelected && !isCorrectAnswer && <span className="text-xs font-semibold text-rose-600 uppercase tracking-wider">Your Selection</span>}
                        {isSelected && isCorrectAnswer && <span className="text-xs font-semibold text-emerald-600 uppercase tracking-wider">Your Selection</span>}
                      </div>
                    </div>
                  );
                })}
